...
/// Награды временно убраны — список пуст.
///
/// Причина. App Store, разбирая заявку, спросил про платное содержимое:
/// единственным местом во всём приложении, где упоминались деньги, была
/// награда «скидка 1000 сом при оплате курса у устаза». Цен нет больше
/// нигде — ни в курсах, ни на сервере. Отвечать про скидку значило открыть
/// вторую тему, Guideline 3.1.1: то, что открывает содержимое ВНУТРИ
/// приложения, положено продавать через встроенные покупки, а уроки лежат
/// именно внутри.
///
/// Коины при этом продолжают начисляться и копиться — экран показывает
/// баланс и прошлые выкупы. Вернуть награды можно одной строкой; вещи,
/// которые устаз выдаёт на руки (тасбих, книга, намазлык), под правило
/// о встроенных покупках не подпадают вовсе, их опознаватели на сервере
/// сохранены:
///
...
///   ShopItem('course', 'Скидка на курс',
///       'Скидка 1000 сом при оплате курса у устаза', 1000,
