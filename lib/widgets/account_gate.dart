...
///
/// **Правило одно: под замком лежит то, у чего без учётной записи нет
/// смысла.** Это трекер намазов (отметки — личная запись ученика, она уходит
/// устазу и переезжает между телефонами), коины и награды у устаза, курсы с
/// уроками (доступ выдаёт устаз конкретному ученику), сертификаты (бланк
/// выписывается на имя) и сообщения в чате эфира (уходят под именем из
/// профиля, по нему же работают жалоба и «скрыть автора»).
///
/// Всё, что считается на самом телефоне и никуда не отправляется, открыто без
/// регистрации: время намаза, Коран, кибла, настройки, азкары и дуа, Рамадан,
/// 99 имён, счётчик зикров, новости и просмотр эфира. Так требует App Store
/// Review 5.1.1(ii) — просить личные данные ради того, что и так работает,
/// нельзя, и приложение из-за этого заворачивают.
///
/// Закрытые разделы **не прячутся, а показываются с замком**: иначе человек не
/// узнает, что в приложении вообще есть курсы и сертификаты, и регистрироваться
/// ему будет незачем.
class AccountGate {
  const AccountGate._();
...
class _InviteSheet extends StatelessWidget {
  ...
  static const _perks = [
    ...
    (Icons.monetization_on_outlined, 'Коины за намазы и зикры'),
    (Icons.school_outlined, 'Курсы и уроки от устаза'),
    (Icons.workspace_premium_outlined, 'Сертификаты за пройденные модули'),
    (Icons.chat_bubble_outline, 'Сообщения в чате эфира'),
  ];
      ...
      child: SafeArea(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              t('Чтение, зикры, азкары, новости и эфир работают и без него.'),
...
/// Заглушка вместо закрытой страницы в свайпах (сейчас это только трекер).
///
/// Страница остаётся на своём месте, чтобы не переставлять индексы всего
