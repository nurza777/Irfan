...
# Список может существовать, но быть пустым — «( )», поэтому ищем запись
# внутри. Раньше искали почту, но Xcode 26 хранит там только UUID аккаунта —
# проверка ложно срабатывала на машине, где вход давно выполнен.
